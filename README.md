...
- ✅ **维云翻牌独立任务**（每 2 小时整点翻 1 次；每个 `vsllm.com` 账号每轮翻 1 次，并先激活 50% 加成）
- ✅ **维云答题/广告奖励任务**（答题每天 1 次、广告每天最多 3 次；每次领取成功后立即翻牌 1 次）
- ✅ **翻牌记录与每日状态仪表盘**（全账号累计、单账号统计、答题状态、视频进度、奖品分布、每日趋势和最近 500 条明细）
- ✅ **安全手动翻牌入口**（从首页前往 GitHub Actions 确认执行，不在公开页面保存 Cookie 或 Token）
- ✅ 错误处理和超时控制
- ✅ 支持手动触发和定时任务
- ✅ **钉钉通知**（签到完成后自动推送结果）
...
`.github/workflows/gwent.yml` 使用 `0 */2 * * *`，在 UTC 和北京时间的每个偶数整点触发，每个账号每轮翻牌 1 次。定时运行会生成稳定的 UTC 两小时槽 token，Worker 通过 KV 租约和槽位记录防止同一槽位重复执行；手动运行则使用独立 token 并强制执行。任务结束后即使历史上报失败，Worker 槽位防重仍会阻止定时任务重复消费。翻牌日志可在 Actions 的 Summary 或 Artifacts 中查看，额度和明细也会同步到首页仪表盘。

该工作流优先使用仓库 Secret `ACTIONS_TOKEN`，其值应与 Worker 的同名 Secret 一致；未配置时会回退到现有 `CONFIG_AUTH`，用于兼容两个令牌原本就相同的部署。

答题和广告任务沿用同一份 `CONFIG_URL`/`NEWAPI_ACCOUNTS` 配置，不需要新增 Cookie Secret。对应工作流分别是 **维云每日答题** 和 **维云广告任务**；它们与签到、普通翻牌共享并发组。答题在北京时间每天 00:15 执行 1 次，答对并领取奖励后立即翻牌 1 次。广告工作流在北京时间奇数整点轮询，每天最多成功领取 3 次，每次领取成功后立即翻牌 1 次；达到次数上限或仍在冷却时只跳过当前轮询。答题、广告和普通翻牌的结果都会写入同一份首页历史记录。

答题和广告工作流还会把每个账号的脱敏每日状态上报到 Worker：答题显示已完成、待执行或异常，视频显示 `0/3` 至 `3/3`、可执行/冷却/已完成状态和下一次可看时间。状态使用独立 KV 文档保存，不会把没有翻牌的轮询计入翻牌次数、额度或历史 run。北京时间换日后，首页不会继续把昨天的完成状态当成今天。

首页的 **手动翻牌** 按钮只会打开仓库的 `gwent.yml` GitHub Actions 页面；登录且拥有仓库权限后，再点击 **Run workflow** 完成最终确认。每次手动运行会对所有 `vsllm.com` 账号各尝试翻牌 1 次，并先激活 50% 加成。手动运行不会推迟下一次两小时整点任务，连续触发也会分别执行，因此可能提前消耗可用次数。公开页面不会读取、保存或传递账号 Cookie、`ACTIONS_TOKEN` 或 GitHub PAT。
