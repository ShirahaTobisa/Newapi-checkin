...
- ✅ **维云翻牌独立任务**（北京时间 01:05 起每 6 小时 5 分钟触发；每个 `vsllm.com` 账号最多翻 3 次，每次先激活 50% 加成）
- ✅ **维云答题/广告加成任务**（答题和观看广告只领取额外翻牌次数，由定时翻牌工作流统一消费）
...
`.github/workflows/gwent.yml` 在北京时间 01:05、07:05、13:05、19:05 触发。Worker 会用 KV 租约记录最近一次任务申请和完成时间，只有距离上一轮至少 6 小时 5 分钟才执行；任务结束后即使历史上报失败，租约仍会阻止重复运行。翻牌日志可在 Actions 的 Summary 或 Artifacts 中查看，额度和明细也会同步到首页仪表盘。

该工作流优先使用仓库 Secret `ACTIONS_TOKEN`，其值应与 Worker 的同名 Secret 一致；未配置时会回退到现有 `CONFIG_AUTH`，用于兼容两个令牌原本就相同的部署。

答题和广告任务沿用同一份 `CONFIG_URL`/`NEWAPI_ACCOUNTS` 配置，不需要新增 Cookie Secret。对应工作流分别是 **维云每日答题** 和 **维云广告任务**；它们与签到、普通翻牌共享并发组。任务只领取额外翻牌次数并记录当前余额，不调用翻牌接口；下一次普通翻牌工作流会按每个账号最多 3 次的计划消费可用次数。
