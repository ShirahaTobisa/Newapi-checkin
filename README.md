...
当前已部署的 `newapi-sync.mornye.uk` 实例使用的是 D1 中的生产覆盖值：

| 任务 | 当前生产计划 | 说明 |
|---|---|---|
| 签到 | 每天 01:00 | 每账号每天 1 次 |
| 答题 | 每天 01:10 | 成功后翻牌 1 次，每天最多 1 次 |
| 常规翻牌 | 从 01:05 起每 120 分钟 | 每账号 1 次，先激活 50% 分享加成 |
| 视频任务 | 从 01:15 起每 120 分钟检查 | 每天最多 3 次，领取成功后翻牌 1 次 |

VSLLM 状态接口目前返回 `cooldown_seconds=7200`（2 小时），因此调度间隔配置为 120 分钟；`01:05` 和 `01:15` 是错峰起始时间，不是额外增加 5 分钟的间隔。上游返回的翻牌额度按 50% 分享加成计入统计，展示值使用原始额度乘以 `1.5`。

生产计划会保存在 D1 中，后续如通过控制台修改，应以 `GET /api/admin/settings` 和 `GET /api/dashboard` 的实时结果为准；上表生产值最后核验于 2026-07-19。
...
管理员余额查询为只读接口 `GET /api/admin/balances`，会返回每个 VSLLM 账号的 `available`、`charges_current`、`charges_max`、`cooldown_seconds` 和 `ad_min_interval_seconds`。查询余额不会执行签到、答题、视频或翻牌任务。
